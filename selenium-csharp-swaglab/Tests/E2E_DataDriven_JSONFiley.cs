/*Data driven from passing global data using [TestCaseSource] NUnit annotation/Attribute but wrap into method AddTEstDataConfig using TestCaseData object*/
...
public class E2E_DataDriven_JSONFile : Base
{
    ...
    public void E2ETest(string username, string password, string sortItem, string firstname, string lastname, string postalCode)
    {
        ...
        login.Login(username, password);        //TestCase global data fromexternal json file
        ...
        Thread.Sleep(2000); 
        inventory.SortItemsLowToHigh(sortItem);     
    ...
    {
        // Ensure that getDataParser is initialized before use
        if (getDataParser == null)
        {
            getDataParser = new TestDataReader(); // Initialize if null
        }

        //Read test data from JSON using testDataReader from Base
        string username = getDataParser.ExtractData("user.username");
        string password = getDataParser.ExtractData("user.password");

        yield return new TestCaseData(username, password, "lohi", "user", "test", "12456");
        yield return new TestCaseData(username, password, "hilo", "user", "test", "34567");
    }
}
